...
static bool vlc_http_header_value_is_valid(const char *value)
{
    if (value == NULL)
        return false;
    /* Prevent request header injection (CR/LF are not allowed in field-values). */
                                         ...
                                         const char *custom_headers)
{
    ...
    while (*p != '\0')
    {
        ...
        if (!vlc_http_header_value_is_valid(value))
        {
            if (res->logger)
                msg_Dbg(res->logger, "HTTP custom header skipped (invalid value): %s", name);
            free(name);
            free(value);
            continue;
        }
        ...
        if (vlc_http_custom_headers_add(res, name, value) && res->logger)
            msg_Dbg(res->logger, "HTTP custom header added: %s", name);
...
vlc_http_res_req(const struct vlc_http_resource *res, void *opaque)
{
    ...
    for (size_t i = 0; i < res->custom_headers_count; i++)
    {
        ...
        if (res->logger)
            msg_Dbg(res->logger, "HTTP request custom header: %s",
                    res->custom_headers[i].name);
    }
